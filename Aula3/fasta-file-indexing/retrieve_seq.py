...
def main():
    ...
    parser_splice.add_argument('-f', '--file', dest='file', action="store", help="A multifasta file",
                        required=False)
    parser_splice.add_argument('-r', '--range', action="store", help="A values in the form start-end space separated. 10-20 50-60 70-100",
    ...
    if hasattr(args, 'start') and args.start is not None:   # args.start exists and has a value
        ...
        gene_name = args.gene
        arte = search_fasta.search(fasta, start, end, gene_name)
