...
def create_table(conn, logger):
    ...
    try:
        c.execute('CREATE TABLE IF NOT EXISTS chipseq('
    ...
    try:
        with conn:
            for data in list_of_data:
                ...
                c.execute("INSERT INTO chipseq VALUES("
                          ":cell_type_category,"
                          ":cell_type,"
...
def update_assay(conn, assay, assay_new, logger):
    ...
    try:
        with conn:
            c.execute("UPDATE chipseq SET assay = :assay_new  WHERE assay = :assay", {'assay': assay,
                                                                                       'assay_new': assay_new})
            logger.info(f'Assay: {assay} was updated for assay_new: {assay_new}')

    except sqlite3.OperationalError:
        logger.error(f'COLD NOT UPDATE Assay: {assay} for assay: {assay_new}')
...
def update_donor(conn, donor, donor_new, logger):
    ...
    try:
        with conn:
            c.execute("UPDATE chipseq SET donor = :donor_new  WHERE donor = :donor", {'donor': donor,
                                                                                       'donor_new': donor_new})
            logger.info(f'Donor: {donor} was updated for donor_new: {donor_new}')

    except sqlite3.OperationalError:
        logger.error(f'COLD NOT UPDATE Donor: {donor} for donor: {donor_new}')
...
def select_assay_track_name(conn, assay_track_name, logger):
    c = conn.cursor()

    try:
        with conn:
            c.execute("SELECT track_name FROM chipseq WHERE assay_track_name = :assay_track_name",
                      {"assay_track_name": assay_track_name})
            all_track = c.fetchall()
...
def select_assay(conn, assay, logger):
    ...
    try:
        with conn:
            c.execute("SELECT cell_type_track_name, assay_track_name, track_name, "
                      "track_type, track_density FROM chipseq WHERE assay = :assay", {"assay": assay})
        ...
        logger.error(f'Could not Select chipseq assay. Check if the table exists.')
...
def select_assay_cell_type(conn, assay, logger):
    ...
    try:
        with conn:
            c.execute("SELECT cell_type FROM chipseq WHERE assay = :assay",
                      {"assay": assay})
            all_cell_type = c.fetchall()
            logger.info(f'Selected chipseq with assay: {assay}')
